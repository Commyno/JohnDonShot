...
enum GameState{
    case NoState        // Attualmente nessuno stato
    case WaitingState   // Necessita di stato
    case Loading        // In caricamento
    case Start          // In gioco
    case Settings       // Settaggio
}
...
extension UISlider{
    ...
    func moveIn()  {
        ...
        anim.duration = 0.5
        anim.fromValue = [self.frame.origin.x + (self.frame.size.width/2) + ScreenSize.width,
                        self.frame.origin.y + (self.frame.size.height/2)]
        anim.toValue = [self.frame.origin.x + (self.frame.size.width/2),
                        self.frame.origin.y + (self.frame.size.height/2)]
